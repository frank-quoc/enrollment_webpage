...
class GetAndPost(Resource): 
    ...
    # POST
    def post(self):
        data = api.payload  # payload is from api_restx, so bottom fields provided by payload
...
class GetUpdateDelete(Resource): 
    ...
    def put(self, idx):
        data = api.payload
        User.objects(user_id=idx).update(**data) # **: packing upacking
        return jsonify(User.objects(user_id=idx))
...
def login():
    ...
    if form.validate_on_submit(): # if no errors
        ...
        if user and user.get_password(password):
            ...
            session['user_id'] = user.user_id # saves the login info for session
...
def register(): 
    ...
    if form.validate_on_submit():
        ...
        user = User(user_id=user_id, email=email, first_name=first_name, 
            last_name=last_name)
